    ...
    <>
      <Head>
        ...
        <style jsx global>{`
          ...
          .animate-tick {
            animation: tick 1s cubic-bezier(0.4, 0, 0.2, 1) infinite;
      ...
      <main className="flex min-h-screen flex-col items-center justify-center bg-[#222222] text-white">
        ...
        <img 
          src={gameOver ? "exploded.svg" : "bomb.svg"} 
          width={gameOver ? "192px" : "128px"} 
          className={`${(gameStart && !gameOver) ? "animate-tick" : ""} heat-up`}
          style={{
            display: "inline-block",
            filter: gameStart && !gameOver ? 
              `sepia(${Math.min(100, (5 - timeLeft) * 20)}%) 
               saturate(${Math.min(300, 100 + (5 - timeLeft) * 40)}%) 
               hue-rotate(${Math.min(-30, (5 - timeLeft) * -6)}deg)
               brightness(${Math.min(130, 100 + (5 - timeLeft) * 6)}%)` 
              : 'none'
          }}
        />
        </div>
